...
describe('--help option', () => {
  ...
  it('it prints `server` command in command list', () => {
    // then
    expect(output).toMatch(
      /server \[options\]\s+Start local web server and display the list of design system components/
    );
  });

  it('it prints `push` command in command list', () => {
    // then
    expect(output).toMatch(/push \[options\]\s+Upload design system repository information to UXPin/);
  });
