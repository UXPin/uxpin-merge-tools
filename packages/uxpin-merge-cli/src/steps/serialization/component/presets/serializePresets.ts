...
export function serializePresets(bundle: PresetsBundle, infos: ComponentPresetInfo[]): PresetsSerializationResult {
  const aggregator: PresetsSerializationResult = { result: [], warnings: [] };
  return infos.map(thunkSerializePreset(bundle)).reduce((result, presetSerializationResult) => {
    const results: any[] = [];
    const warnings: any[] = [];
