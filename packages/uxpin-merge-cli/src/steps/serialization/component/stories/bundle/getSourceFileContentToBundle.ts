...
import { getUniqStorySetImportName } from './getUniqStoryImportName';
...
function thunkGetImport(tempDirPath:string):({ path }:ComponentPresetInfo) => string {
  return ({ path }) => (
    `import * as ${getUniqStorySetImportName(path)} from '${relative(tempDirPath, path)}';`
  );
}

function getExport({ path }:ComponentPresetInfo):string {
  return `  ${getUniqStorySetImportName(path)},`;
}

function flattenComponentPresetInfos(components:ComponentDefinition[]):ComponentPresetInfo[] {
  return flatMap(components, ({ info }) => (info.stories || []));
}
